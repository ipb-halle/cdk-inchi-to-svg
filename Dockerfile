...
# Install packages
RUN apt-get -y update && apt-get -y dist-upgrade && apt-get -y --allow-unauthenticated install apt-transport-https make gcc g++ maven openjdk-8-jdk-headless openjdk-8-jre-headless
...
# Cleanup
RUN apt-get -y --purge --auto-remove remove make gcc g++ openjdk-8-jdk-headless maven && \
